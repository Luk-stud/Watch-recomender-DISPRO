...
config = {
    ...
    'min_lr': 0.0000001,
    'max_images_per_brand': 300  # Maximum number of images per brand before oversampling
}
...
def create_base_vgg_model():
    ...
    for layer in vgg.layers[:]:  # Unfreeze the last 4 layers of the VGG16 model
...
weights_path = 'claify_family.h5'  # Path to the weights file
load_model_weights(model, weights_path)

# Rebuild the classification layer based on the updated classes
x = model.layers[-4].output  # Assuming -4 is the last layer before the classification layer
...
save_path = "tuned_brand_clasifyer_from_family.h5"
# Save the model
