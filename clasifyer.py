...
def filter_valid_brands(df, min_brands=3, min_images_per_brand=5):
    valid_rows = []
    grouped = df.groupby('Brand')
    for brand, group in grouped:
        if len(group) >= min_images_per_brand:
...
def split_by_brand(df):
    ...
    for brand, group in grouped:
        images = group['Image Path'].values
        if len(images) >= 5:  # Ensure there are at least 5 images to split into train, val, test
            train_images, test_images = train_test_split(images, test_size=0.2, random_state=13)
            train_images, val_images = train_test_split(train_images, test_size=0.25, random_state=13)  # 0.25 * 0.8 = 0.2
        else:  # All data goes to train if there are fewer than 5 images
            train_images = images
            val_images = images
            test_images = images
        
        train_rows.append(group[group['Image Path'].isin(train_images)])
        val_rows.append(group[group['Image Path'].isin(val_images)])
        test_rows.append(group[group['Image Path'].isin(test_images)])
...
class WatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = int(self.df.iloc[idx]['Brand'])
...
class WatchClassificationModel(nn.Module):
    def __init__(self, num_classes, base_model_path=None):
        ...
        if base_model_path:
            ...
            # Load the weights without the classifier head
            self.base_model.load_state_dict({k: v for k, v in state_dict.items() if 'classifier' not in k}, strict=False)
...
def train(model, train_loader, optimizer, criterion, device, epochs, val_loader, scheduler=None, log_interval=1, patience=5):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "best_brand_model.pth")
        else:
            patience_counter += 1
...
def main():
    ...
    # Filter out brands with fewer entries
    ...
    df = filter_valid_brands(df)

    train_df, val_df, test_df = split_by_brand(df)
    ...
    model = WatchClassificationModel(num_classes=len(brand_encoder.classes_), base_model_path="best_brand_model.pth")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=wandb.config.learning_rate)  # Changed to Adam optimizer
    ...
    trained_model = train(model, train_loader, optimizer, criterion, device, wandb.config.epochs, val_loader, scheduler)
