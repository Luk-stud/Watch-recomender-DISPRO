...
def train(model, train_loader, train_dataset, optimizer, criterion, device, epochs, val_loader, log_interval=10):
...
def main():
    ...
    wandb.init(project='contrastive_loss', entity='DISPRO2', config={
        ...
        "epochs": 5,
        "batch_size": 32,
        "margin": 0.7,  # Adjusted margin for CosineContrastiveLoss
        "loss_type": "CosineContrastiveLoss"  # Dynamic loss type configuration
    })
    train_dataset = PairDataset(train_data, save_dir=saved_pairs_dir, dataset_type="train",max_pairs= 50000)
    ...
    # Adjust the loss criterion based on wandb configuration
    if wandb.config.loss_type == "CosineContrastiveLoss":
        criterion = CosineContrastiveLoss(margin=wandb.config.margin)
    elif wandb.config.loss_type == "ContrastiveLoss":
        criterion = ContrastiveLoss(margin=wandb.config.margin, loss_scale=3)
    else:
        raise ValueError("Unsupported loss type specified!")
    ...
    # Directly visualize embeddings from specified brands
